...
class FraudDetectionXGBoost:
    ...
    def _optimize_threshold(self, y_true, y_prob):
        """
        Optimize threshold to maximize recall while maintaining precision
        """
        precisions, recalls, thresholds = precision_recall_curve(y_true, y_prob)
        # Add the endpoint (1.0) since precision_recall_curve doesn't include it
        thresholds = np.append(thresholds, 1.0)
        
        valid_points = recalls >= 0.80 
        if not np.any(valid_points):
            print("Warning: No threshold achieves base recall. Using highest recall threshold.")
            best_idx = np.argmax(recalls)
        else:
            # Among points with sufficient recall, maximize precision
            ...
            print(f"Recall: {recalls[valid_points][best_idx]:.4f}")
            print(f"Precision: {valid_precisions[best_idx]:.4f}")   
            return threshold
        ...
        with mlflow.start_run(nested=True, run_name="xgboost_tuning", experiment_id=
            ...
            for params in self._generate_param_combinations():
                ...
                for train_idx, val_idx in tscv.split(X_train):
                    ...
                    cv_class_weights = self._calculate_class_weight(y_cv_train)
                    cv_sample_weights = np.array([cv_class_weights[y] for y in y_cv_train])
                    ...
                    model = xgb.XGBClassifier(
                        **params,
                        tree_method='hist',
                        eval_metric=['aucpr'],
                        scale_pos_weight = cv_class_weights[1]/cv_class_weights[0]
                    )
                    model.fit(
                    ...
                    valid_points = precisions >= 0.65
                    if np.any(valid_points):
                        valid_recalls = recalls[valid_points]
                        f2_scores = (5 * precisions[valid_points] * valid_recalls) / (2 * precisions[valid_points] + valid_recalls)
